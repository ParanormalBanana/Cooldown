...
internal sealed class MainViewModel : ObservableObject
{
    ...
    public MainViewModel()
    {
        ...
        OpenLibraryCommand = new RelayCommand(_ => OpenLibraryPage());
    ...
    public bool RescanEnabled => !Scanning;
    public bool ShowProgress { get => _showProgress; set { if (Set(ref _showProgress, value)) Raise(nameof(ShowLibrary)); } }
    public bool ShowLibrary => !_showProgress;
    ...
    public bool ShowHidden { get => _showHidden; private set { if (Set(ref _showHidden, value)) Raise(nameof(ShowHiddenLabel)); } }
    public string ShowHiddenLabel => ShowHidden ? "Hide hidden games" : "Show hidden games";
    ...
    public ImageSource? ModalCover => _selected?.Cover;
    public string PutOnPrompt => $"Do you want to put {SelectedName} on cooldown?";
    ...
    public void SetViewportWidth(double width)
    {
        ...
        var gap = 12.0;
        var cardW = Math.Max(170, (inner - cols * gap) / cols);
    ...
    {
        var installedIds = _games.Select(g => g.Id).ToHashSet();
        var listed = new List<Game>();
        var seen = new HashSet<string>();
        foreach (var entry in _state.Cooldowns)
        {
            if (IsIgnored(entry.Game)) continue;
            if (seen.Add(entry.Game.Id) && !installedIds.Contains(entry.Game.Id))
        ...
        var onCooldown = _state.Cooldowns.Select(c => c.Game.Id).ToHashSet();
        bool Matches(Game g) =>
        ...
        GameItem Make(Game g) =>
            new(g, installedIds.Contains(g.Id), onCooldown.Contains(g.Id), hidden.Contains(g.Id));
    ...
    private void Rescan()
    {
        ...
        Task.Run(() =>
        {
            ...
            _ui?.BeginInvoke(() => ApplyGames(games));
        });
    }
    ...
    private void ApplyGames(List<Game> games)
    {
        _games = games;
    ...
    {
        _selected = item;
        _active = _state.Cooldowns.FirstOrDefault(c => c.Game.Id == item.Id);
        Raise(nameof(SelectedName));
        Raise(nameof(ModalCover));
        Raise(nameof(PutOnPrompt));
        Raise(nameof(TakeOffPrompt));
        item.EnsureCover((int)Math.Round(CardWidth * 2));
    ...
    private void PutOnCooldown()
    {
        ...
        var installed = _games.Any(g => g.Id == _selected.Id);
    ...
    public void MarkNotAGame(GameItem item)
    {
        ...
        _state.Cooldowns = _state.Cooldowns
            .Where(c => !c.Game.Id.Equals(item.Id, StringComparison.OrdinalIgnoreCase))
            .ToList();
        _games = _games.Where(g => !g.Id.Equals(item.Id, StringComparison.OrdinalIgnoreCase)).ToList();
